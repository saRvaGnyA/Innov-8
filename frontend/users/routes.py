...
def addEvent():
    ...
    if request.method == 'POST':
        ...
        sql_query = '''INSERT INTO Event(event_type_id,organizer_id,event_start_date,event_end_date,event_location,event_description) values ({},1,'{}','{}',"{}","{}");'''.format(
            int(EventType), Eventstartdate, Eventenddate, EventLocation, EventDescription)
...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('''SELECT * FROM {} WHERE {} = "{}"'''.format(
            table_info[Usertype][0], table_info[Usertype][2], Email))
        user2 = cursor.fetchall()
        users = Users.query.filter_by(
            email=form.email.data, type_of_user=Usertype).first()
...
def register():
    ...
    if request.method == 'POST':
        ...
        Email = form.email.data
        Password = bcyrpt.generate_password_hash(
